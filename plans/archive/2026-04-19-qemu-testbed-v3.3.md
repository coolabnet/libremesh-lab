...
- [ ] **1.1 Create build script** at `scripts/qemu/build-libremesh-image.sh` that:
  - Follows the LibreMesh build process from https://libremesh.org/development.html:
  ...
  - **Creates a defconfig** at `scripts/qemu/libremesh-testbed.defconfig` (NOT interactive `make menuconfig`):
    ...
    ```
  - **Note:** `netcat` is needed by adapter scripts for babeld fallback (`collect-nodes.sh:139`, `collect-topology.sh:118`). `iw` is needed for WiFi interface checks. `lime-system` and `lime-proto-bmx7` are needed for LibreMesh community config that `validate-node.sh` checks. The defconfig `CONFIG_PACKAGE_*` names may need adjustment — run `make defconfig` and verify with `grep CONFIG_PACKAGE_ .config`.
  - Build sequence:
    ```bash
    cp scripts/qemu/libremesh-testbed.defconfig .config
    make defconfig  # expand defconfig to full .config
    ./scripts/feeds update -a
    ./scripts/feeds install -a
    make -j$(nproc)
    ```
...
- [ ] **2.1 Create vwifi-server management script** at `scripts/qemu/start-vwifi.sh`:
  ...
  - Launches in TCP mode bound to bridge IP: `bin/vwifi-server -u -a 10.99.0.254` (`-u`: use-port-in-hash for multi-VM identification; `-a 10.99.0.254`: bind to bridge IP — vwifi defaults to 127.0.0.1 per `src/config.h`, MUST override for VMs to reach it)
...
- **Test command:**
  ...
  ssh -i run/ssh-keys/id_ed25519 root@10.99.0.11 'ping6 -c 3 -I eth0 fe80::5054:ff:fe00:2' 2>/dev/null || true
  ```
- **Expected:** IPv4 ping: `3 packets transmitted, 3 received, 0% packet loss`
- **Note:** IPv6 link-local addresses use the MAC-based format: `fe80::5054:ff:fe00:02` for VM2's MAC `52:54:00:00:00:02`. If IPv4 ping fails but SSH works, the issue is likely firewall rules on the management interface — LibreMesh may have iptables rules that block ICMP. In that case, SSH connectivity is sufficient proof of inter-VM communication.
...
- **IPv6 parser fix required:** `collect-topology.sh:203` only matches IPv4 (`\d+.\d+.\d+.\d+`). BMX7 in LibreMesh may use IPv6 link-local addresses. If `node_count` stays at 1 (self only), apply this fix to the adapter script:
  ```bash
  # Current (IPv4 only):
...
4. **Phase 2.1: Fixed vwifi-server bind address** — Added `-a 10.99.0.254` flag since vwifi-server defaults to `127.0.0.1` (from `config.h`). VMs on 10.99.0.0/16 bridge cannot reach localhost-bound server.
...
Also added: dual-IP scheme clarification (management 10.99.0.x vs mesh LibreMesh-configured), vwifi-server port confirmation (8212 TCP/INET from config.h).
