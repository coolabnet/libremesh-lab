...
if [[ "${FORCE_REBUILD}" == "false" ]] && [[ -f "${CACHED_HASH_FILE}" ]]; then
    ...
    if [[ "${CURRENT_HASH}" == "${CACHED_HASH}" ]]; then
        ...
        # Verify symlink exists and is valid; restore if missing
        SYMLINK="${OUTPUT_DIR}/libremesh-x86-64.ext4"
        if [[ ! -e "${SYMLINK}" ]]; then
            log "Symlink missing or broken, restoring..."
            # Find the latest .img.gz
            LATEST_GZ="$(find "${OUTPUT_DIR}" -maxdepth 1 -name 'libremesh-x86-64-*.img.gz' -type f | sort | tail -1)"
            if [[ -n "${LATEST_GZ}" ]]; then
                DEST_IMG="${LATEST_GZ%.gz}"
                # Decompress if .img doesn't exist alongside .img.gz
                if [[ ! -f "${DEST_IMG}" ]]; then
                    log "Decompressing ${LATEST_GZ}..."
                    gunzip -fk "${LATEST_GZ}"
                fi
                ln -sf "$(basename "${DEST_IMG}")" "${SYMLINK}"
                # The .img.gz artifact is the unconfigured build output
                # (no SSH keys, no DHCP leases). Refuse to leave the user
                # with an unbootable image — tell them what to run.
                die "Restored symlink from unconfigured cache.
...
log "Decompressed: ${DEST_IMG}"

# ─── Image size verification (Gate 1.1: uncompressed must be >50MB) ──────────
IMAGE_SIZE=$(stat -c%s "${DEST_IMG}" 2>/dev/null || stat -f%z "${DEST_IMG}")
...
image_file: "$(basename "${DEST_IMG}")"
image_file_compressed: "${DEST_NAME}"
...
log "Build complete!"
log "  Image (compressed): ${OUTPUT_DIR}/${DEST_NAME}"
log "  Image (uncompressed): ${DEST_IMG}"
log "  Manifest: ${OUTPUT_DIR}/build-manifest.yaml"

# ─── Post-build: configure image for testbed ────────────────────────────────────
log "Configuring image for testbed..."
if ! bash "${SCRIPT_DIR}/configure-source-image.sh" --image "${DEST_IMG}"; then
    die "Image configuration failed. The image would be unconfigured (no SSH keys, no DHCP), which makes the testbed unusable. Re-run configure-source-image.sh manually after fixing the underlying error: bash scripts/qemu/configure-source-image.sh --image ${DEST_IMG}"
fi
